...
contract RandomGame is VRFConsumerBase, Ownable {
    ...
    //Var to indicate whether the game has begun or not
    ...
    //event emmitted when game begins
    event GameStarted(uint256 gameId, uint8 maxPlayers, uint256 entryFee);

    //event emmitted when someone joins a game
    event PlayerJoins(uint256 gameId, address player);

    //event emmitted when game ends
    event GameEnded(uint256 gameId, address gameWinner, bytes32 reqId);

    /*Constructor inheriting VRFConsumer and initiating the vals for keyHash, fee and gameStarted */
